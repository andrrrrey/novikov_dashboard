...
1. Квиз — 3 вопроса, по одному на каждый аспект (Маркетинг / Продажи / Менеджмент).
   Каждый вариант ответа явно соответствует уровню по шкале 1..N, поэтому уровень
   аспекта — это уровень выбранного варианта (маппинг вариант→уровень живёт в
   quiz_data.py).
...
def evaluate(levels_by_aspect: dict[Aspect, int]) -> dict:
    ...
    Вход:  {Aspect.MARKETING: 2, Aspect.SALES: 1, Aspect.MANAGEMENT: 3} — по
           одному уровню (шкала 1..N) на аспект.
    ...
    if any(not isinstance(lvl, int) or lvl < 1 for lvl in levels_by_aspect.values()):
        raise ValueError("Уровень аспекта должен быть положительным целым")
